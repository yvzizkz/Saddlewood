...
import { money, pctStr, type TrackerSummary } from '@/lib/trackers/core'
import { fmt } from './format'

type Props = { s: TrackerSummary; paymentsCount: number }

const VALUE = 'text-[clamp(17px,4.6vw,24px)] text-[var(--color-charcoal)] tabular-nums leading-tight whitespace-nowrap'
...
function Tile({ label, value, sub, children }: { label: string; value: string; sub?: string; children?: React.ReactNode }) {
  return (
    <div className="rounded-lg border bg-white px-4 py-3 min-w-0" style={{ borderColor: 'var(--color-stone)' }}>
      <div className="text-[10px] tracking-[0.12em] uppercase mb-1.5 leading-tight" style={{ color: 'var(--color-charcoal-light)' }}>
        {label}
      </div>
      <div className={VALUE} style={{ fontFamily: 'var(--font-fraunces)' }}>
        {value}
      </div>
      {sub ? (
...
export function TrackerKpis({ s, paymentsCount }: Props) {
  ...
  const heroSub =
    s.sumG > 0
      ? `Work billed ${fmt(s.sumCp)}${s.sumDraw > 0.005 ? ` · advance draws ${fmt(s.sumDraw)}` : ''}${carry > 0.005 ? ` · prior balance ${fmt(carry)}` : carry < -0.005 ? ` · less credit ${fmt(-carry)}` : ''}`
      : carry > 0.005
        ? `Prior balance carried ${fmt(carry)}`
        : carry < -0.005
          ? `Credit on account ${fmt(-carry)}`
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      <div className="col-span-2 md:col-span-4 rounded-lg px-5 py-4" style={{ backgroundColor: 'var(--color-teal)', color: 'var(--color-cream)' }}>
        ...
        <div className="text-[clamp(32px,9vw,48px)] tabular-nums leading-none" style={{ fontFamily: 'var(--font-fraunces)' }}>
          {fmt(s.totalDue)}
        </div>
      ...
      <Tile label="Paid to date" value={fmt(s.paid)} sub={`${paymentsCount} ${paymentsCount === 1 ? 'payment' : 'payments'}`} />
