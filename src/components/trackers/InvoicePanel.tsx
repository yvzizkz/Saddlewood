...
import { DUE_RED, fmt } from './format'
...
export function InvoicePanel({ state, calcs, s, lines, busy, result, trackerId, onGenerate, onPaid }: Props) {
  ...
  if (Math.abs(carry) > 0.005) items.push({ name: carry > 0 ? 'Prior balance carried' : 'Credit carried', kind: '', amt: carry })
  ...
  return (
    <section aria-labelledby="inv-heading" className="rounded-lg border bg-white p-4" style={{ borderColor: 'var(--color-stone)' }}>
      ...
      <h2 id="inv-heading" className="text-xl text-[var(--color-charcoal)] mt-0.5" style={{ fontFamily: 'var(--font-fraunces)' }}>
      ...
      <div className="mt-3 flex flex-col gap-1.5">
        {items.length === 0 ? (
          <p className="text-sm rounded-lg border border-dashed px-3 py-3 text-center" style={{ borderColor: 'var(--color-stone-mid)', color: 'var(--color-charcoal-light)' }}>
            Nothing on this invoice yet. Bill a line below.
          </p>
        ) : (
          items.map((it, i) => (
      ...
      <div className="mt-4 flex flex-col gap-2">
        <button
          ...
          onClick={onGenerate}
          disabled={busy !== null}
          className="w-full py-3 rounded-xl bg-[var(--color-teal)] text-[var(--color-cream)] font-semibold min-h-[48px] disabled:opacity-60 inline-flex items-center justify-center gap-2"
        >
        ...
        <button
          ...
          onClick={onPaid}
          disabled={busy !== null}
          className="w-full py-3 rounded-xl border-2 font-semibold min-h-[44px] disabled:opacity-60"
          style={{ borderColor: 'var(--color-stone)', color: 'var(--color-charcoal)' }}
        >
          Client paid #{state.invoice.number}…
        </button>
        <a
          href={`/api/trackers/${trackerId}/workbook`}
          className="w-full py-2.5 rounded-xl text-sm font-medium min-h-[40px] inline-flex items-center justify-center gap-2"
      ...
      <p className="mt-3 text-xs leading-relaxed" style={{ color: 'var(--color-charcoal-light)' }}>
        {issued
          ? `Issued${state.invoice.issuedAt ? ` ${new Date(state.invoice.issuedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}` : ''}. Generating again re-sends the workbook with any changes.`
