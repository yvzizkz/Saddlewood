  ...
  return (
    <section aria-labelledby="notes-heading" className="rounded-lg border bg-white" style={{ borderColor: 'var(--color-stone)' }}>
      <div className="px-4 pt-4 pb-2 flex items-start justify-between gap-3">
        <div>
          <h2 id="notes-heading" className="text-xl text-[var(--color-charcoal)]" style={{ fontFamily: 'var(--font-fraunces)' }}>
            Notes
          </h2>
        ...
        <label className="inline-flex items-center gap-2 text-sm shrink-0 min-h-[40px]" style={{ color: 'var(--color-charcoal)' }}>
          <input type="checkbox" checked={mode === 'custom'} onChange={(e) => onMode(e.target.checked ? 'custom' : 'auto')} className="size-4 accent-[var(--color-gold-accessible)]" />
          Write my own
        </label>
      </div>
      <div className="px-4 pb-4">
        {mode === 'custom' ? (
          <textarea
            ...
            className="w-full min-h-[220px] rounded-lg border px-3 py-2 text-sm leading-relaxed outline-none focus:border-[var(--color-teal)]"
