...
import { fmt, H2, whenIso } from './format'
  ...
  return (
    <section aria-labelledby="hist-heading" className="rounded-lg border bg-white" style={{ borderColor: 'var(--color-stone)' }}>
      <div className="px-4 pt-4 pb-2">
        <h2 id="hist-heading" className={H2} style={{ fontFamily: 'var(--font-fraunces)' }}>
          Invoice history
        </h2>
      ...
      <div className="px-4 pb-3 flex flex-col">
        {invoices.length === 0 ? (
          <p className="text-sm py-3" style={{ color: 'var(--color-charcoal-light)' }}>
            No invoices generated from the portal yet.
          </p>
        ) : (
          invoices.map((inv) => (
            <div key={inv.id} className="py-2 border-t flex flex-wrap items-center gap-x-4 gap-y-1 text-sm" style={{ borderColor: 'var(--color-stone)' }}>
              ...
              <span className="flex-1 min-w-[180px]" style={{ color: 'var(--color-charcoal-light)' }}>
                ...
                {inv.status === 'superseded' ? ' · recorded at payment, not generated' : inv.sentTo ? ` · sent to ${inv.sentTo}` : ' · generated, not emailed'} · {whenIso(inv.generatedAt, true)}
                {inv.generatedBy ? ` by ${inv.generatedBy.split('@')[0]}` : ''}
              </span>
              <span className="flex items-center gap-1">
                <a href={`/api/trackers/${trackerId}/workbook?invoice=${encodeURIComponent(inv.invoiceNumber)}`} className="inline-flex items-center gap-1 underline min-h-[44px] px-2" style={{ color: 'var(--color-teal)' }}>
                  <Download className="size-3.5" aria-hidden="true" /> Workbook
                </a>
                <button type="button" disabled={busy} onClick={() => onRestore(inv.invoiceNumber)} className="underline min-h-[44px] px-2 disabled:opacity-50" style={{ color: 'var(--color-charcoal)' }}>
                  Restore
                </button>
              </span>
            </div>
