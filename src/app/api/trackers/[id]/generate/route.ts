...
import { lineCalc, overBilledLines, summary, unpricedBilledLines, workbookFilename } from '@/lib/trackers/core'
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    if (!state.invoice.number) return NextResponse.json({ ok: false, error: 'Give the invoice a number first.' }, { status: 400 })
    const unpriced = unpricedBilledLines(state, calcs)
    if (unpriced.length) {
    ...
    if (!input.force) {
      const over = overBilledLines(state, calcs)
      if (over.length) {
    ...
    const snapshot = await saveInvoiceSnapshot({ trackerId: id, state: tracker.state, actor: who.actor, sentTo: sentTo.join(', ') || null, emailId })
