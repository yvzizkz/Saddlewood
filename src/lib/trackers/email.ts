...
export function buildTrackerEmail(state: TrackerState, opts: { portalUrl: string; generatedBy: string; filename: string }): TrackerEmail {
  ...
  const rows: EmailRow[] = [
    ...
    { label: 'Paid to date', value: `${money2(s.paid)} (${state.payments.length} payments)` },
  ...
  const intro = `The tracker workbook for invoice #${inv.number} is attached (${opts.filename}), built from the numbers saved in the portal a moment ago. Open it in Numbers or Excel to check it over, then forward it to ${state.project.owners || 'the owners'}.`
